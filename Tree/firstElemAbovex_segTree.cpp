...
struct segTree{
    ...
    int first_above_x(int v,int x,int lx,int rx){
        if(tree[x]<v) return -1;
        if(rx-lx==1) return lx;

        int m=(lx+rx)/2;
        int res=first_above_x(v,2*x+1,lx,m);
        if(res==-1){
            res=first_above_x(v,2*x+2,m,rx);
        }
        return res;
    }


    int first_above_x(int v){
        return first_above_x(v,0,0,size);
    }
